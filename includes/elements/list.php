...
class listItems extends Comet_Element {
	...
	public function view(){
		...
		if( comet.helpers.isString( data.el.sty ) && ( tmp = data.el.sty.trim() ) !== 'none' ){
			classes += ' cpb-list' + tmp;

		}
		o = '<ul class="' + classes + '">';
		o += comet.helpers.foreachItem( data, function( iid, idata ){
			var iclasses = 'cpb-item cpb-inner';
			iclasses += ' cpb-item' + iid;
			iclasses += ' ' + comet.helpers.sanitizeAlignment( idata.alg );

			return '<li class="' + iclasses + '">' + ( comet.helpers.isString( idata.ctnt ) ? idata.ctnt : '' ) + '</li>';
	...
	public function css(){
		...
		sty = comet.helpers.isString( data.el.sty ) ? data.el.sty.trim() : 'none';
		spa = comet.helpers.sanitizeNumber({ value: data.el.spa, min: 0, max: 200 });
		ti = comet.helpers.sanitizeNumber({ value: data.el.ti, min: 0, max: 200 });

		if( ( tmp = comet.css.margin( data.el.mrt, data.el.mrr, data.el.mrb, data.el.mrl, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-list.cpb-wrapper', tmp );

		}
		css = '';

		if( ( tmp = comet.helpers.sanitizeNumber({ value: data.el.fs, min: 10, max: 100 } ) ) !== null ){
		...
		if( spa !== null && ti !== null && ( spa > 0 || ti > 0 ) ){
			css += comet.css.padding( spa, 0, spa, ti, 'px', 'px' );

		}
		o += comet.css.element( id, '.cpb-item.cpb-inner', css );
		...
		o += comet.helpers.foreachItem( data, function( iid, idata ){
			...
			if( sty === 'img' ){
				img = comet.helpers.isString( idata.ico ) ? ( comet.helpers.stripTags( idata.ico ) ).trim() : '';

				if( img !== '' ){
					icss = comet.css.property( 'background-image', 'url(' + comet.helpers.escUrl( img ) + ')' );
					io = comet.css.element( id, '.cpb-inner.cpb-item' + iid, icss );
		...
		if( ( tmp = comet.css.margin( data.el.mrtt, data.el.mrrt, data.el.mrbt, data.el.mrlt, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-list.cpb-wrapper', tmp, 't' );
			o += comet.css.responsive( 't', comet.css.element( id, '.cpb-list.cpb-wrapper', tmp ) );

		}

		if( ( tmp = comet.css.margin( data.el.mrtm, data.el.mrrm, data.el.mrbm, data.el.mrlm, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-list.cpb-wrapper', tmp, 'm' );
			o += comet.css.responsive( 'm', comet.css.element( id, '.cpb-list.cpb-wrapper', tmp ) );
