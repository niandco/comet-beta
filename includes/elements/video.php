...
class video extends Comet_Element{
	...
	public function view(){
		...
		const vArray = [ 'true', '1', 'TRUE', 'True' ];
		const pos = comet.helpers.isString( data.el.pos ) ? ( comet.helpers.stripTags( data.el.pos ) ).trim() : '';
		const ov = ( pos !== '' && data.el.ov === 'p' );
		const aut = comet.helpers.inArray( vArray, data.el.aut );
		const inf = comet.helpers.inArray( vArray, data.el.inf );
		const w = comet.helpers.sanitizeNumber({ value: data.el.wi, min: 200, default: 500 });
		const h = comet.helpers.sanitizeNumber({ value: data.el.he, min: 200, default: 400 });
		var url = comet.helpers.isString( data.el.url ) ? ( comet.helpers.stripTags( data.el.url ) ).trim() : '';
		var video = false;
		var o, d, a, cover;
		...
		switch( data.el.type ){
			...
			d = 'preload="metadata"';
			d += aut ? ' autoplay' : '';
			video = '<video class="cpb-video cpb-frame" src="' + comet.helpers.escUrl( url ) + '" height="' + h + '" width="' + w + '" ' + d + '>';
