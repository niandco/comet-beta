...
class map extends Comet_Element {
	...
	public function view(){
		...
		const w = comet.helpers.sanitizeNumber({ value: data.el.wi, min: 200, max: 2000, default: 200 });
		const h = comet.helpers.sanitizeNumber({ value: data.el.he, min: 200, max: 2000, default: 200 });
		var src, tmp, o;

		src = 'https://maps.google.com/maps?output=embed&=iwloc=B';
		if( comet.helpers.isString( data.el.q ) ){
			src += '&q=' + ( comet.helpers.stripTags( data.el.q ) ).trim();

		}
		src += '&z=' + comet.helpers.sanitizeNumber({ value: data.el.zoom, min: 0, max: 21, default: 10 });

		if( comet.helpers.isString( data.el.mt ) && comet.helpers.inArray( [ 'satellite', 's' ], ( tmp = data.el.mt.trim() ) ) ){
			src += '&t=k';
		...
		o += ' src="' + comet.helpers.escUrl( src ) + '" allowfullscreen  scrolling="no" frameborder="0">';
	...
	public function css(){
		...
		if( ( tmp = comet.css.margin( data.el.mrt, data.el.mrr, data.el.mrb, data.el.mrl, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-map.cpb-frame', tmp );

		}

		if( ( tmp = comet.css.margin( data.el.mrtt, data.el.mrrt, data.el.mrbt, data.el.mrlt, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-map.cpb-frame', tmp, 't' );
			o += comet.css.responsive( 't', comet.css.element( id, '.cpb-map.cpb-frame', tmp ) );

		}

		if( ( tmp = comet.css.margin( data.el.mrtm, data.el.mrrm, data.el.mrbm, data.el.mrlm, 'px', 'px' ) ) !== '' ){
			o += comet.css.element( id, '.cpb-map.cpb-frame', tmp, 'm' );
			o += comet.css.responsive( 'm', comet.css.element( id, '.cpb-map.cpb-frame', tmp ) );
